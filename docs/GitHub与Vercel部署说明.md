...
浏览器访问 Vercel 时，`127.0.0.1` 表示访问者自己的电脑，不是部署模型的电脑。当前生产前端可以连接访问者本机的 `8011` 服务：同一台电脑先启动 BioCLIP，再打开 Vercel 即可使用联合识别；没有安装或启动模型的电脑只显示服务未就绪，不会生成伪候选。`start_bioclip.ps1` 已将正式 Vercel 域名加入跨域白名单。

这仍然是“在线前端 + 本地模型”的部署方式。Vercel 不保存 8 GB 模型，也不会代替本机运行推理；若要让多名同学直接共享同一模型，需要把 Python 服务部署到校内 GPU 主机或其他受控服务器，并把 `VITE_VISION_API_URL` 改为该服务的 HTTPS 地址。
