...
本项目是本地运行的 React + Vite 植物地理学实习平台，包含特征图谱、289 条植物知识库、文字与图片联合识别、图片样本补录、实习教学文档和可配置模型服务网关。

V2.2 新增可追溯名称校订与常见别称检索。接受名用于页面展示，常见别称、旧拉丁名和历史误写仍可用于名录与语义识别搜索。
...
```powershell
npm run curate:names
```

每日导入器只挂接正式名录中可确认的名称；名录外名称进入待复核池。重复执行同一文件不会重复添加图片。

导入器只写入 `public/data` 与派生资产。原始工作簿、群聊图片和已有的 785 张 iPlant 参考图不会被修改。19 个原有图片空缺使用带作者、许可和原始链接的 GBIF 社区参考图补齐，目前 289 种植物均有本地可加载图片。群聊派生图会剥离 EXIF，未匹配图片只进入待复核池。

## 原型边界

联合识别支持只输入文字、只上传图片或同时使用两种证据。文字候选来自本地结构化字段与反向索引；本机 BioCLIP v1.7 启动后，文字还会作为性状约束参与图片候选重排。模型不可用时只执行文字检索和本地图像检查，不显示虚假物种候选。
